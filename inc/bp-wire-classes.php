...
class BP_Wire_Post {
	...
	function get_last_updated($component,$component_id){
		global $bp,$wpdb;
                $table_name=$bp->{$component}->table_name_wire;

                $date=$wpdb->get_var($wpdb->prepare("select date_posted from {$table_name} where item_id=%d order by date_posted DESC limit 1",$component_id));
		return $date;
	}
